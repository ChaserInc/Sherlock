...
var Sherlock = (function () {
  var patterns = {
      rangeSplitters:
        /(\bto\b|\-|\b(?:un)?till?\b|\bthrough\b|\bthru\b|\band\b|\bends?\b)/g,
      ...
      // 2019/5/27, 2019-5-27
      shortFormY:
        /\b(\d{4})[\/\-](0?[1-9]|1[0-2])[\/\-]([1-2]\d|3[0-1]|0?[1-9])\b/g,

      // tue, tues, tuesday
      weekdaysStr:
        "\\b(sun|mon|tue(?:s)?|wed(?:nes)?|thu(?:rs?)?|fri|sat(?:ur)?)(?:day)?\\b",
      relativeDateStr:
        "((?:next|last|this) (?:week|month|year)|tom(?:orrow)?|tmrw|tod(?:ay)?|(?:right )?now|tonight|day after (?:tom(?:orrow)?|tmrw)|yest(?:erday)?|day before yest(?:erday)?)",
      inRelativeDateStr: "(\\d{1,4}|a) (day|week|month|year)s? ?(ago|old)?",

      inRelativeTime:
        /\b(\d{1,2} ?|a |an )(h(?:our|r)?|m(?:in(?:ute)?)?)s? ?(ago|old)?\b/,
      ...
      explicitTime:
        /(?:@ ?)?\b(?:at |from )?(1[0-2]|[0-2]?[1-9])(?::?([0-5]\d))? ?([ap]\.?m?\.?)?(?:o'clock)?\b/,

      more_than_comparator:
        /((?:more|greater|older|newer) than|after|before) \$(?:DATE|TIME)\$/i,
    ...
    nowDate = null,
    getNow = function () {
      if (nowDate) return new Date(nowDate.getTime());
      else return new Date();
    },
    readConfig = function (config_var) {
      return typeof Watson !== "undefined" && Watson.config
        ? Watson.config[config_var]
        : null;
    },
    parser = function (str, time, startTime) {
      ...
      ret.isAllDay = !!(
        dateMatch &&
        !timeMatch &&
        !dateMatch.match(/^(?:right )?now$|^tonight$/)
      );
    ...
    matchTime = function (str, time, startTime) {
      ...
      if ((match = str.match(new RegExp(patterns.explicitTime.source, "g")))) {
        ...
        if (match.length <= 2 && str.trim().length > 2) matchConfidence = 0;
        else {
          ...
          if (
            ...
            str.match(patterns.internationalTime)
          ) {
            ...
            if (meridian.indexOf("p") === 0 && hour != 12) hour += 12;
            else if (meridian.indexOf("a") === 0 && hour == 12) hour = 0;
            matchConfidence += 20;
      ...
      if ((match = str.match(patterns.monthDay))) {
        ...
        if (
          (match.indexOf("on") !== 0 &&
            ...
            !(
              match.indexOf("the") === 0 &&
              match.indexOf(",", match.length - 1) !== -1
            ) &&
    ...
    makeAdjustments = function (start, end, isAllDay, str, ret) {
      ...
      if (end) {
        if (
          ...
          helpers.isSameDay(start, end) &&
          helpers.isSameDay(start, now)
        ) {
        ...
        if (ret.eventTitle.match(patterns.more_than_comparator)) {
          if (
            (start <= now &&
              (!helpers.isSameDay(start, now) || str.match(/ago|old\b/)) &&
          ...
          ret.eventTitle = ret.eventTitle.replace(
            patterns.more_than_comparator,
            ""
          );
        }
    ...
    helpers = {
      ...
      strToNum: function (str) {
        return str.replace(patterns.digit, function (val) {
          ...
          if (val.indexOf("th", val.length - 2) !== -1) out += "th";
          else if (val.indexOf("st", val.length - 2) !== -1) out += "st";
          else if (val.indexOf("nd", val.length - 2) !== -1) out += "nd";
          else if (val.indexOf("rd", val.length - 2) !== -1) out += "rd";
          return out;
        });
      },
  ...
  // may 5, may 5th
  patterns.monthDay = new RegExp(
    patterns.months + " " + patterns.days + "(?: " + patterns.years + ")?"
  );
  // 5th may, 5 may
  patterns.dayMonth = new RegExp(
    patterns.days +
      "(?: (?:day )?of)? " +
      patterns.months +
      "(?: " +
  ...
  patterns.digit = new RegExp(
    "\\b(" + helpers.intToWords.join("|") + ")\\b",
    "g"
  );
  // today, tomorrow, day after tomorrow
  patterns.relativeDate = new RegExp("\\b" + patterns.relativeDateStr + "\\b");
  // in 2 weeks
  patterns.inRelativeDate = new RegExp(
    "\\b" + patterns.inRelativeDateStr + "\\b"
  );
  // 2 weeks from tomorrow
  patterns.inRelativeDateFromRelativeDate = new RegExp(
    "\\b" +
      patterns.inRelativeDateStr +
      " from " +
      patterns.relativeDateStr +
      "\\b"
  );

  // next Friday, thu
  patterns.weekdays = new RegExp(
    "(?:(next|last) (?:week (?:on )?)?)?" + patterns.weekdaysStr
  );
  // oxt monday
  patterns.oxtDays = new RegExp(
    "(?:\\boxt|\\bweek next) " + patterns.weekdaysStr
  );
  // thursday week
  ...
  return {
    ...
    parse: function (str) {
      ...
      var date = getNow(),
        ...
        result =
          typeof Watson !== "undefined" ? Watson.preprocess(str) : [str, {}],
        str = result[0],
        ret = result[1];
      ...
      var tokens = readConfig("disableRanges")
        ? [str.toLowerCase()]
        : str.toLowerCase().split(patterns.rangeSplitters);
      ...
      while (!ret.endDate) {
        if (tokens.length > 1) {
          ...
          if ((result = parserRet) !== null) {
            ...
            if (
              result.eventTitle.replace(/\$(?:DATE|TIME)\$/g, "").length >
              ret.eventTitle.replace(/\$(?:DATE|TIME)\$/g, "").length
            )
              ret.eventTitle = result.eventTitle;
      ...
      if (ret.eventTitle) {
        ...
        var fillerWords = readConfig("disableRanges")
          ? patterns.fillerWords2
          : patterns.fillerWords;
        ret.eventTitle = ret.eventTitle.split(fillerWords)[0].trim();
        ret.eventTitle = ret.eventTitle
          .replace(
            /(?:^| )(?:\.|-$|by$|in$|at$|from$|on$|starts?$|for$|(?:un)?till?$|!|,|;)+/g,
            ""
          )
          .replace(/ +/g, " ")
          .trim();
        var match = str.match(
          new RegExp(helpers.escapeRegExp(ret.eventTitle), "i")
        );
        if (match) {
